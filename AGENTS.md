...
- 승/패 조건 검증
- 점수 계산 검증
- 발견된 버그의 재현 스크립트 및 회귀 테스트 요구사항 명시
...
- 스펙 모호성 지적
- 밸런스 개선 제안 (선택)
- 버그별 회귀 테스트 스크립트 실행 결과 (`tests/bugs/*.test.mjs`)
...
- 상태 머신 무시 구현 금지
- QA 생략 금지
- 버그 수정 후 회귀 테스트 스크립트 미작성/미실행 금지

---

# 11. Bug Regression Test Rule

- 버그가 재현되면 먼저 `tests/bugs/`에 재현 가능한 테스트 스크립트를 추가한다.
- 스크립트 파일명은 `bug-keyword.test.mjs` 형식을 사용한다.
- 수정 커밋에는 반드시 아래 순서를 포함한다.
  1. 실패하는 재현 테스트 작성
  2. 코드 수정
  3. 동일 스크립트 재실행 후 PASS 확인
- QA 보고서에는 실행 명령과 PASS/FAIL 결과를 기록한다.
