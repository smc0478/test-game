...
## 1) Design Goal
v1.4.0은 기존 전투 로직을 유지하면서 **캔버스 기반 전장 연출과 하단 전투 조작 레이아웃**을 도입해 실제 대전처럼 보이는 GUI를 강화한다.
...
- 전투 UI 개선:
  - 캔버스 기반 VS 전장(배경/그라운드/캐릭터 렌더링) 적용.
  - 턴 전환 시 플레이어/적 공격 펄스 애니메이션 제공.
  - 상단 헤더 버튼과 별개로 손패 근처에 하단 전투 조작 바(턴 종료 + 안내문) 배치.
  - 카드 이미지를 포함한 기존 손패 UI 유지.
- 시너지/저장/명예의 전당 기능 유지.
