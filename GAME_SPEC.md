...
## 1) Design Goal
v2.1.0은 **카드 풀을 비우고(0장), 카드 컨셉을 사용자 지정 입력 기반으로 다시 설계할 준비 상태**로 전환한다.
...
- 카드 풀은 비움(`CARD_LIBRARY = {}`), 시작 덱은 빈 배열로 설정.
- 카드 데이터가 없는 상태에서도 런타임이 중단되지 않도록 적 덱 카드 매핑은 무효 카드 제거 규칙을 적용.
- 시너지/버스트 밸런스 로직은 유지한다(카드 재입력 전까지 비활성 상태).
- 런 길이는 50라운드를 유지한다.
...
- 카드 풀: 총 0장(초기화 상태)
- 신규 카드 추가는 사용자 지정 컨셉 승인 후 순차 반영
