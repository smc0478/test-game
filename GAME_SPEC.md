...
## 1) Design Goal
v2.6.2는 공허 흡혈 카드의 회복 판정을 실제 체력 피해 기준으로 통일해 방어도 상대로 과회복이 발생하지 않도록 조정한다.
...
- 카드 풀을 36장으로 구성한다(기본 16장 + 화염 컨셉 5장 + 리프 컨셉 5장 + 성운 드로우 컨셉 5장 + 공허 흡혈 컨셉 5장).
- 신규 공허 흡혈 컨셉 5종을 추가한다.
  1. 다음 공격의 실제 피해량만큼 흡혈을 부여하는 카드
  2. 이번 턴 회복량을 증가시키는 카드
  3. 에너지 대신 체력을 지불해 사용하는 흡혈 공격 카드
  4. 잃은 체력 비례 피해를 주는 카드
  5. 공격마다 실제 피해량 기반 흡혈을 부여하는 카드
...
- 적 덱 임시 규칙:
  - 첫 적 1종은 기본 카드 16장(C001~C016)으로 고정.
  - 다음 적 4종은 문양별 기본 카드(Flame/Leaf/Gear/Void)만 사용.
  - 그다음 적 4종은 문양 컨셉 카드 + 동일 문양 기본 카드 조합 사용.
  - 나머지 적은 현재 카드 풀(C001~C036)에서 랜덤 덱을 사용.
...
- 신규 성운 드로우 컨셉 카드:
  ...
  - C031: `영점 추출` (코스트 2, 드로우 2 + 드로우 카드 코스트 1 감소)

- 신규 공허 흡혈 컨셉 카드:
  - C032: `혈인 각인` (다음 공격의 실제 피해만큼 흡혈)
  - C033: `공허 수혈` (회복량 +2 증폭 + 회복 4)
  - C034: `피의 계약` (코스트 0, 체력 4 지불 + 피해 13 + 실제 피해량 흡혈)
  - C035: `결손 포식` (잃은 체력 x2 피해, 최대 24)
  - C036: `심연 송곳니` (이번 턴 공격마다 실제 피해량 흡혈 + 드로우 1)

제한:
- Gear 카드의 드로우+에너지 동시 폭주 금지 원칙 유지.
- 카드 효과는 순수 로직 연산으로 처리.
- 체력 코스트 카드는 현재 HP가 비용 이하일 때 사용할 수 없다.
...
### v2.6.2 (2026-02-14)
- 공허 흡혈 카드의 회복 판정을 실제 체력 피해 기준으로 통일했다(방어도 소모량은 흡혈 제외).
- `피의 계약(C034)`을 실제 피해량 흡혈 규칙으로 조정했다.
- `심연 송곳니(C036)`를 공격마다 실제 피해량 기반 흡혈 버프로 조정했다.

### v2.6.0 (2026-02-13)
- 공허 문양에 흡혈 컨셉 카드 5장(C032~C036)을 추가해 체력-흡혈 리스크/리턴 빌드를 확장했다.
- 신규 효과 타입(`grantNextAttackLifestealFromDamage`, `increaseHealPower`, `payHpCost`, `attackPerMissingHp`, `grantLifestealOnAttack`)을 도입했다.
- 체력 코스트 카드는 체력이 비용 이하일 때 사용 불가 규칙을 명시했다.

### v2.6.1 (2026-02-14)
- 적 덱을 임시 단계형 규칙(1개 코어16 → 4개 기본 문양 → 4개 컨셉 조합 → 나머지 랜덤)으로 재편했다.
- 후속 컨셉 적 제작 전까지 모든 적이 유효 카드(C001~C036)만 사용하도록 덱 구성을 정규화했다.

### v2.5.0 (2026-02-13)
- 기어 문양에 성운 드로우 컨셉 카드 5장(C027~C031)을 추가해 드로우-템포 선택지를 확장했다.
- 신규 효과 타입(`drawByFamilyCount`, `attackPerHandCard`, `redrawHandTo`, `gainNextTurnEnergy`, `drawThenDiscount`)을 도입했다.
- 카드 아트는 기존 SVG 데이터 URL 생성 방식을 그대로 유지했다.
