...
## 1) Design Goal
v2.5.0은 기존 26장 카드 풀에 **성운 드로우 컨셉 카드 5장**을 추가해 Gear 문양의 드로우-코스트 템포 빌드를 확장한다.
...
- 카드 풀을 31장으로 구성한다(기본 16장 + 화염 컨셉 5장 + 리프 컨셉 5장 + 성운 드로우 컨셉 5장).
- 신규 성운 드로우 컨셉 5종을 추가한다.
  1. 같은 패밀리 사용 횟수만큼 추가 드로우를 얻는 카드
  2. 현재 손패 장수 배수만큼 공격하는 카드
  3. 현재 손패를 전부 버리고 5장을 새로 뽑는 카드
  4. 드로우 대신 다음 턴 에너지를 예약하는 고유 카드
  5. 코스트 2로 드로우 후 뽑은 카드 코스트를 낮추는 카드
